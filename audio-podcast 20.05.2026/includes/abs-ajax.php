...
function abs_ajax_remove_book_progress() {
    ...
    if ($type === 'audio') {
        $wpdb->delete($wpdb->prefix . 'abs_progress', array('user_id' => $user_id, 'book_id' => $book_id));
    } else {
        $wpdb->delete($wpdb->prefix . 'abs_reading_progress', array('user_id' => $user_id, 'ranobe_id' => $book_id));
    }
