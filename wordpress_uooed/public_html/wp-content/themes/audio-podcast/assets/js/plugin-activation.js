jQuery(document).ready(function($) {
    'use strict';
    var this_obj = audio_podcast_plugin_activate_plugin;


    $(document).on('click', '.audio-podcast-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message audio-podcast-plugin-install');
                button.addClass('button-primary audio-podcast-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.audio-podcast-recommended-plugins .audio-podcast-activation-note').css('display','block');
    ...
    $(document).on('click', '.audio-podcast-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .audio-podcast-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .audio-podcast-recommended-plugins').css('display','none');
    });
