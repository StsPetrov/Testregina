<?php
$post_id = get_the_ID();
$abs_book_id = get_post_meta($post_id, 'abs_book_id', true);

$meta = array('author' => '', 'genres' => array(), 'cover_url' => '', 'description' => '', 'ranobe_id' => 0);
if ($abs_book_id) $meta = abs_get_book_meta_from_ranobe($abs_book_id);

global $wpdb;
$total_seconds = $wpdb->get_var($wpdb->prepare("SELECT SUM(total_progress_seconds) FROM {$wpdb->prefix}abs_progress WHERE book_id = %s", $abs_book_id));
$total_hours = $total_seconds > 0 ? round($total_seconds / 3600, 1) : 0;

$total_duration = '';
if ($abs_book_id) {
    $total_duration = $wpdb->get_var($wpdb->prepare("SELECT total_duration_formatted FROM {$wpdb->prefix}abs_book_cache WHERE book_id = %s", $abs_book_id));
}

$ranobe_permalink = $meta['ranobe_id'] ? get_permalink($meta['ranobe_id']) : '';
?>
...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    <div class="ranobe-header" style="display:flex; gap:20px; margin-bottom:20px;">
        <div class="ranobe-cover" style="flex-shrink:0; width:200px;">
            <?php if ($meta['cover_url']): ?>
                <img src="<?php echo esc_url($meta['cover_url']); ?>" style="border-radius:12px; width:100%;">
            <?php else: ?>
        ...
        <div class="ranobe-info" style="flex:1;">
            <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:5px;">
                ...
                <button id="abs-favorite-btn" class="favorite-btn" style="background:transparent; border:none; font-size:2.5rem; cursor:pointer; color:rgba(255,255,255,0.5); flex-shrink:0;">♡</button>
            </div>
            <div class="book-rating" id="book-rating" style="display:flex; align-items:center; gap:10px; margin-bottom:10px; flex-wrap:wrap;">
                <span style="color:#ffc107;">Рейтинг:</span>
                <div class="stars" id="rating-stars" style="display:flex; gap:3px;">
                ...
                <span class="rating-value" id="rating-value" style="color:#ffc107; font-weight:bold;">0</span>
                <span class="rating-count" id="rating-count" style="color:rgba(255,255,255,0.5); font-size:0.8rem;">(0)</span>
                <select id="book-status-select" style="background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:white; padding:6px 12px; border-radius:20px; margin-left:10px;">
                    <option value="listening">🎧 Слушаю</option>
                    <option value="completed">✅ Прослушано</option>
...
jQuery(document).ready(function($) {
    $('#rating-stars .star').on('click', function() {
        ...
        $('#rating-stars .star').each(function(i) {
            $(this).toggleClass('active', i < rating);
            $(this).text(i < rating ? '★' : '☆');
        });
        $.post(absPlayerData.ajaxUrl, {
            action: 'save_abs_rating',
            book_id: '<?php echo $abs_book_id; ?>',
            rating: rating
        }, function(r) {
            if (r.success) {
                $('#rating-value').text(Number(r.data.average).toFixed(1));
                $('#rating-count').text('(' + r.data.count + ')');
            }
        });
    });
    
    var checkRating = setInterval(function() {
        if (typeof absPlayerData !== 'undefined') {
            $.get(absPlayerData.ajaxUrl, {
                action: 'get_abs_rating',
                book_id: absPlayerData.itemId
            }, function(r) {
                if (r.success && r.data) {
                    $('#rating-value').text(Number(r.data.average).toFixed(1));
                    $('#rating-count').text('(' + r.data.count + ')');
                    $('#rating-stars .star').each(function() {
                        ...
                        $(this).toggleClass('active', v <= Math.round(r.data.average));
                        $(this).text(v <= Math.round(r.data.average) ? '★' : '☆');
                    });
                }
            });
            ...
            <?php if ($meta['author']): ?>
                <p style="color:#0dcaf0; font-size:1.1rem; margin:0 0 10px;">
                    ✍️ <a href="/catalog?author=<?php echo urlencode($meta['author']); ?>" style="color:#0dcaf0; text-decoration:none;"><?php echo esc_html($meta['author']); ?></a>
                </p>
            <?php endif; ?>
            ...
            <?php if (!empty($meta['genres'])): ?>
                <div style="margin-bottom:10px;">
                    <?php foreach ($meta['genres'] as $genre): ?>
                        <a href="/catalog?genre=<?php echo urlencode($genre); ?>" style="display:inline-block; padding:4px 12px; background:rgba(13,202,240,0.2); color:#0dcaf0; border-radius:20px; font-size:0.8rem; text-decoration:none; margin:2px;"><?php echo esc_html($genre); ?></a>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
            ...
            <div style="display:flex; gap:15px; flex-wrap:wrap; margin-bottom:15px;">
                <?php if ($total_duration): ?>
                    <span style="background:rgba(255,255,255,0.1); padding:4px 12px; border-radius:20px; font-size:0.9rem;">⏱ <?php echo esc_html($total_duration); ?></span>
                <?php endif; ?>
                <?php if ($total_hours > 0): ?>
                    <span style="background:rgba(13,202,240,0.15); padding:6px 14px; border-radius:20px; font-size:0.9rem;">
                        🎧 Прослушали: <strong><?php echo $total_hours; ?> ч</strong>
                    </span>
                <?php endif; ?>
            </div>
        </div>
    ...
    <?php if ($ranobe_permalink): ?>
    <div style="background:linear-gradient(135deg,rgba(13,202,240,0.1),rgba(91,192,222,0.1)); border:1px solid rgba(13,202,240,0.25); border-radius:12px; padding:16px 20px; margin:15px 0; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <span style="font-size:2rem;">📖</span>
        <div style="flex:1;">
            <div style="color:#fff; font-weight:600; font-size:1rem; margin-bottom:4px;">Читайте с комфортом</div>
            <div style="color:rgba(255,255,255,0.7); font-size:0.85rem; line-height:1.4;">Текстовая версия книги с удобной читалкой, закладками и ночным режимом — всё для полного погружения в историю.</div>
        </div>
        <a href="<?php echo esc_url($ranobe_permalink); ?>" style="background:linear-gradient(90deg,#0dcaf0,#5bc0de); color:#1b2039; padding:10px 20px; border-radius:25px; text-decoration:none; font-weight:600; font-size:0.9rem; white-space:nowrap;">📖 Читать книгу</a>
    </div>
<?php endif; ?>
...
<?php echo abs_generate_book_seo_text(get_post_meta(get_the_ID(), 'abs_book_id', true), 'audio'); ?>
