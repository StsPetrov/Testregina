<?php
/**
 * Админка парсера ifreedom v2 — полная версия
 * Фильтры, сортировки, настройки, выбор книг, cron
 */
...
// ========== AJAX: Настройки ==========
add_action('wp_ajax_abs_ifreedom_v2_save_settings', 'abs_ifreedom_v2_ajax_save_settings');
function abs_ifreedom_v2_ajax_save_settings() {
    check_ajax_referer('abs_ifreedom_v2_nonce');
    if (!current_user_can('manage_options')) wp_send_json_error();
    $s = abs_ifreedom_v2_get_settings();
    foreach (['min_delay_ms','max_delay_ms','max_per_minute','cron_batch_size','manual_batch_size','http_timeout'] as $f) {
        if (isset($_POST[$f])) $s[$f] = absint($_POST[$f]);
    }
    update_option('abs_ifreedom_v2_settings', $s);
...
function abs_ifreedom_v2_ajax_process() {
    ...
    for ($i = $sc - 1; $i < $be; $i++) {
        ...
        $chapter_id = abs_ifreedom_v2_save_chapter($pid, $num, $cd['title'], $cd['content'], $cd['volume'] ?? 0);


        // Сохраняем текст главы в MinIO
$s3_key = $pid . '/' . $num . '.html';
s3_put_object(S3_BUCKET_CHAPTERS, $s3_key, $cd['content']);
...
function abs_ifreedom_v2_admin_page() {
    ...
    $all_slugs = $wpdb->get_col("SELECT slug FROM $t WHERE status IN('new','error') $order");
    ...
    ?></div>
    <div style="background:#fff;border:1px solid #ddd;border-radius:8px;margin:20px 0;padding:15px;">
        <h3 onclick="jQuery(this).next().toggle()" style="cursor:pointer;">⚙️ Настройки ▼</h3>
        <form id="v2-settings" style="display:none;display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:15px;">
            <?php foreach(['min_delay_ms'=>'Мин. пауза','max_delay_ms'=>'Макс. пауза','max_per_minute'=>'Запросов/мин','cron_batch_size'=>'Батч Cron','manual_batch_size'=>'Батч ручной','http_timeout'=>'Таймаут HTTP'] as $k=>$l)
                echo '<div><label>'.$l.'</label><br><input type="number" name="'.$k.'" value="'.$settings[$k].'" style="width:100%;"></div>';
            ?>
    ...
    <script>(function($){
        ...
        $('#btn-parse-filtered').click(function(){ if(running)return; if(!slugs.length){alert('Нет книг для загрузки.');return;} if(!confirm('Загрузить '+slugs.length+' книг пакетами по '+CHUNK_SIZE+'?'))return; startParsing(slugs); });
        $('#btn-parse-selected').click(function(){ if(running)return; var s=$('.book-checkbox:checked').map(function(){return this.value;}).get(); if(!s.length)return; if(!confirm('Загрузить '+s.length+' книг пакетами по '+CHUNK_SIZE+'?'))return; startParsing(s); });
        var CHUNK_SIZE = 3;
var chunks = [];
var currentChunk = 0;
...
function startParsing(slugsArray) {
    chunks = chunkArray(slugsArray, CHUNK_SIZE);
    currentChunk = 0;
    if (chunks.length === 0) {
        log('Нет книг для загрузки.');
        return;
    }
    running = true;
    log('📦 Всего пакетов: ' + chunks.length + ' (по ' + CHUNK_SIZE + ' книг)');
    processNextChunk();
}

function processNextChunk() {
    if (currentChunk >= chunks.length) {
        log('✅ Все пакеты обработаны!');
        running = false;
        setTimeout(function() { location.reload(); }, 2000);
        return;
    }
    log('📦 Пакет ' + (currentChunk + 1) + ' из ' + chunks.length + ' (' + chunks[currentChunk].length + ' книг)');
    parse(chunks[currentChunk], 0, 0, 1);
}

function parse(sl, i, pc, sc) {
    $.post(ajaxurl, {
        ...
        if (r.success) {
            var d = r.data;
            prog(d.processed, d.total);
            log(d.log);
            if (d.finished) {
                currentChunk++;
                processNextChunk();
            } else {
                parse(sl, d.next_index, d.processed, d.start_chapter || 1);
            }
        }
    }).fail(function() { log('❌ Ошибка'); running = false; });
}
...
$('#btn-parse-selected').click(function(){ if(running)return; var s=$('.book-checkbox:checked').map(function(){return this.value;}).get(); if(!s.length)return; if(!confirm('Загрузить '+s.length+' книг пакетами по '+CHUNK_SIZE+'?'))return; startParsing(s); });
        ...
        $('#btn-cron-start').click(function(){ if(!confirm('Запустить фон?'))return; $.post(ajaxurl,{action:'abs_ifreedom_v2_cron_toggle',cron_action:'start',book_slugs:[],_ajax_nonce:'<?php echo wp_create_nonce("abs_ifreedom_v2_nonce"); ?>'},function(){updCron();}); });
