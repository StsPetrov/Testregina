...
while (have_posts()) : the_post();
    ...
    <div class="chapter-reader-wrapper">
        <div class="chapter-reader-container">
            ...
            <!-- НАВИГАЦИЯ ПРЕД/СЛЕД (ВЕРХ) -->
<div class="chapter-nav-top">
    <div class="chapter-nav-item">
        <?php if (!empty($prev_chapter)): ?>
            <a href="<?php echo get_permalink($prev_chapter[0]->ID); ?>" class="chapter-nav-btn prev-btn">◀ Предыдущая</a>
        <?php endif; ?>
    </div>
    <div class="chapter-nav-item chapter-nav-right">
        <?php if (!empty($next_chapter)): ?>
            <a href="<?php echo get_permalink($next_chapter[0]->ID); ?>" class="chapter-nav-btn next-btn">Следующая ▶</a>
        <?php endif; ?>
    </div>
</div>
            ...
            <?php endif; ?>
<!-- МОДУЛЬ ЗАКАЗА ОЗВУЧКИ -->
<?php
$chapters_count = count($all_chapters_data);
$price = max(50, ceil($chapters_count * 0.1 / 50) * 50);
?>
<div style="background:linear-gradient(135deg,rgba(255,152,0,0.1),rgba(255,87,34,0.1));border:1px solid rgba(255,152,0,0.3);border-radius:16px;padding:20px;margin:15px auto;text-align:center;max-width:var(--reader-max-width);">
...
document.getElementById('voice-order-btn').addEventListener('click', function() {
    ...
    this.disabled = true;
    var amount = <?php echo $price * 100; ?>;
    var formData = new FormData();
    formData.append('action', 'tbank_init_payment');
    formData.append('amount', amount);
    formData.append('order_id', 'voice_<?php echo $ranobe_id; ?>_' + Date.now());
    formData.append('description', 'Озвучка: <?php echo esc_js($ranobe_title); ?> — <?php echo $chapters_count; ?> глав');
...
<div class="chapter-meta-info">
    ...
    <?php endif; ?>
    <span class="chapter-meta-badge"><?php 
        $text = s3_get_object(S3_BUCKET_CHAPTERS, $ranobe_id . '/' . $chapter_num . '.html');
        if (!$text) $text = get_the_content();
        echo count(preg_split('/\s+/', trim(strip_tags($text)))); 
    ?> слов</span>
</div>
            ...
            <div class="chapter-reader-content" id="reader-content">
              ...
              $s3_key = $ranobe_id . '/' . $chapter_num . '.html';
$content = s3_get_object(S3_BUCKET_CHAPTERS, $s3_key);
if ($content) {
    echo $content;
} else {
    $content = get_the_content();
    echo $content;
            ...
            <!-- КНОПКА ПОДДЕРЖКИ -->
<div style="text-align:center;margin:10px auto;max-width:var(--reader-max-width);">
    <a href="https://pay.cloudtips.ru/p/db763c18" target="_blank" style="display:inline-block;background:linear-gradient(135deg,#ff9800,#ff5722);color:#fff;padding:12px 28px;border-radius:30px;text-decoration:none;font-weight:700;font-size:1rem;">💰 Поддержать проект</a>
</div>
